        ...
        for item in sorted_data:
            ...
            if item.origin_lang_type == '영어':
                ...
                row = (
                    ...
                    item.origin_lang if item.origin_lang else None,
...
#     # 완성된 데이터베이스에서 'origin_lang'과 'pronun_list' 컬럼을 추출하여 pkl 파일로 저장
#     def to_pkl(self, output_file=None):
#         if output_file is None:
#             output_file = self.db_file.replace('.db', '.pkl')

#         conn = sqlite3.connect(self.db_file)
#         cursor = conn.cursor()
#         cursor.execute("SELECT origin_lang, word FROM words")  # 원하는 컬럼들을 선택
#         rows = cursor.fetchall()
#         conn.close()

#         data_to_save = []
#         for row in rows:
#             origin_lang, word = row
#             data_to_save.append({'origin_lang': origin_lang, 'word': word})

#         with open(output_file, 'wb') as pkl:
#             pickle.dump(data_to_save, pkl)

def main():
    to_make_oursam = True
    to_make_ipa = False
    to_make_alphabet = False
